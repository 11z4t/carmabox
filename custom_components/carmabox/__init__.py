...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # PLAT-1242: async_forward_entry_setups raises ValueError on reload
    # when platforms are already registered. Safe to ignore — HA tracks this.
    try:
        await hass.config_entries.async_forward_entry_setups(entry, PLATFORMS)
    except ValueError as err:
        if "has already been setup" in str(err):
            _LOGGER.debug("Platforms already set up, skipping: %s", err)
        else:
            raise
    ...
    # PLAT-1242: register_static_path removed in HA 2025.x.
    # Lovelace card is served via HACS resource loader instead.
    # Kept as no-op to avoid AttributeError on older HA versions.
