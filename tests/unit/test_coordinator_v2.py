...
from custom_components.carmabox.core.coordinator_v2 import (
    CoordinatorConfig, CoordinatorV2, SystemState,
)
...
def _state(**kw):
    defaults = dict(grid_import_w=1500, ellevio_viktat_kw=1.5, pv_power_w=0,
...
class TestPlanExecution:
    def test_discharge_from_plan(self):
        c = CoordinatorV2()
        c._startup_confirmed = True
        c.plan = [PlanAction(hour=23, action="d", battery_kw=-2.0,
                              grid_kw=1.5, price=70, battery_soc=40, ev_soc=60)]
...
class TestNightEV:
    ...
    def test_night_ev_stops_at_target(self):
        ...
        c._startup_confirmed = True
        c.night_ev_active = True
        r = c.cycle(_state(hour=2, ev_soc=76))
        assert not c.night_ev_active

    def test_no_night_ev_daytime(self):
        c = CoordinatorV2()
        c._startup_confirmed = True
        r = c.cycle(_state(hour=14, ev_soc=60, ev_connected=True))
        assert not c.night_ev_active
